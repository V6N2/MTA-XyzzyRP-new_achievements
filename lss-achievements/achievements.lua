...
@new achievements: V6N2 <vinczers@gmail.com>
...
--[[ wzor
	...
	["pedziwiatr"]={ -- By: V6N2
		name="Pędziwiatr",
		descr="Osiągnąłeś poziom staminy równy 999. Możesz być z siebie dumny! +60GP",
		timely=true,
		typ=1,
		value=60,
		timely=true,
		check=function(plr)
			...
			local stamina = getPedStat(plr, 22)
			
			if stamina >= 999 then return true end
			return false
		end
	},
	...
	["strongman"]={ -- By: V6N2
		name="Strongman",
		descr="Osiągnąłeś poziom siły równy 999 jednostek! Od teraz możesz uważać się za prawdziwego kulturyste! +60GP",
		timely=true,
		typ=1,
		value=60,
		timely=true,
		check=function(plr)
			local uid=getElementData(plr,"auth:uid")
			if not uid then return false end
			local sila = getPedStat(plr, 165)
			
			if sila >= 999 then return true end
			return false
		end
	},
	...
	["stalybywalec"]={ -- By: V6N2
		name="Stały bywalec",
		descr="Jesteś z nami już 1000h! Gratulacje! +500GP",
		timely=true,
		typ=1,
		value=500,
		timely=true,
		check=function(plr)
			...
			local r = exports.DB2:pobierzWyniki("select sum(playtime) suma from lss_characters where userid=?", uid)
			if not r or not r.suma then return false end
			if r and tonumber(r.suma) and tonumber(r.suma)>=60000 then return true end
			return false
		end
	},
	
	["1st1000"]={ -- Edited by: V6N2
		...
		check=function(plr)
			local uid=getElementData(plr,"auth:uid")
			if not uid then return false end
			--gotówka w banku i przy sobie
			local sr=exports.DB2:pobierzWyniki("SELECT bank_money FROM lss_characters WHERE id=? LIMIT 1", uid)
			if not sr then return false
			local rm = getPlayerMoney(plr) + sr
			if rm>=1000 then return true end
			return false
		end
	},
	...
	["milioner"]={ -- By: V6N2
		name="Milioner",
		descr="Dorobiłeś się swojego pierwszego milionu! Nie wydaj go aby odrazu! +500GP",
		timely=true,
		typ=2,
		value=500,
		timely=true,
		check=function(plr)
			...
			if not sr then return false
			local rm = getPlayerMoney(plr) + sr
			if rm>=1000000 then return true end
			return false
		end
	},
	...
	["kolekcjoner"]={ -- By: V6N2
		name="Kolekcjoner",
		descr="Jesteś właścicielem 5 pojazdów! Gratulacje +75GP",
		timely=true,
		chance=25,
		typ=3,
		value=75,
		check=function(plr)
			...
			local r = exports.DB2:pobierzWyniki("select count(id) suma from lss_vehicles where owning_player=?", character.id)
			if not r or not r.suma then return false end
			if r and tonumber(r.suma) and tonumber(r.suma)>=5 then return true end
			return false
		end
	},
	...
	["drivlicA"]={
		name="Młody kierowca (A)",
		descr="W końcu otrzymałeś/aś swoje prawo jazdy kategori A! Pora rozejrzeć się za pierwszym motorem +15GP",
		timely=true,
		typ=3,
		value=15,
		check=function(plr)
			local character=getElementData(plr,"character")
			if not character or not character.id then return false end
			local r = exports.DB2:pobierzWyniki("select pjA from lss_characters where id=?", character.id)
			if not r or not r.pjA then return false end
			if r and tonumber(r.pjA) and tonumber(r.pjA)>=1 then return true end
			return false
		end
	},
...
addCommandHandler("xcheckachix", checkTimelyAchievements)
